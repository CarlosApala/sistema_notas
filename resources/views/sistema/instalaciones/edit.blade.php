...
@section('contenido')
<div class="container">
    <h1>Editar Ruta de Instalación</h1>
    ...
    <div class="row">
        <div class="col-md-6">
            <form action="{{ route('instalaciones.update', $rutaInstalacion->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select name="idZona" id="idZona" class="form-select @error('idZona') is-invalid @enderror" required>
                        ...
                        <option value="{{ $zona->id }}" {{ old('idZona', $rutaInstalacion->idZona) == $zona->id ? 'selected' : '' }}>
                            {{ $zona->NombreZona }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="idRuta" id="idRuta" class="form-select @error('idRuta') is-invalid @enderror" required>
                        ...
                        <option value="{{ $ruta->id }}" {{ old('idRuta', $rutaInstalacion->idRuta) == $ruta->id ? 'selected' : '' }}>
                            {{ $ruta->NombreRuta }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="idPredio" id="idPredio" class="form-select @error('idPredio') is-invalid @enderror" required>
                        ...
                        <option value="{{ $predio->id }}" {{ old('idPredio', $rutaInstalacion->idPredio) == $predio->id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nInstalacion" id="nInstalacion"
                        ...
                        value="{{ old('nInstalacion', $rutaInstalacion->nInstalacion) }}" required>
                    @error('nInstalacion')
                    <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
        ...
        <div class="col-md-6">
            ...
            <div id="zonaInfo" style="display:none;">
                <table class="table table-bordered">
                    <tr><th>Zona</th><td id="nombreZona"></td></tr>
                </table>
            </div>

            <div id="rutaInfo" style="display:none;">
                <table class="table table-bordered">
                    <tr><th>Ruta</th><td id="nombreRuta"></td></tr>
                </table>
            </div>

            <div id="predioInfo" style="display:none;">
                <table class="table table-bordered">
                    <tr><th>Dirección</th><td id="direccionPredio"></td></tr>
                    <tr><th>Distrito</th><td id="distritoPredio"></td></tr>
                    <tr><th>Área del Predio</th><td id="areaPredio"></td></tr>
                    <tr><th>Número de Pisos</th><td id="pisosPredio"></td></tr>
                    <tr><th>Tipo Edificación</th><td id="tipoEdificacion"></td></tr>
                    <tr><th>Estado</th><td id="estadoEdificacion"></td></tr>
                </table>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const zonaInfoDiv = document.getElementById('zonaInfo');
        const rutaInfoDiv = document.getElementById('rutaInfo');
        const predioInfoDiv = document.getElementById('predioInfo');

        function updateInfo(select, dataList, updateFields, infoDiv) {
            const selectedId = select.value;
            const data = dataList.find(d => d.id == selectedId);
            if (data) {
        ...
        rutaSelect.addEventListener('change', () =>
            updateInfo(rutaSelect, rutas, { nombreRuta: 'NombreRuta' }, rutaInfoDiv)
        );
        predioSelect.addEventListener('change', () =>
            updateInfo(predioSelect, predios, {
                direccionPredio: 'direccion',
                distritoPredio: 'distrito',
                areaPredio: 'AreaPredio',
                pisosPredio: 'NroPisos',
                tipoEdificacion: 'TipoEdificacion',
                estadoEdificacion: 'EstadoEdificacion',
            }, predioInfoDiv)
        );
        zonaSelect.addEventListener('change', () =>
            updateInfo(zonaSelect, zonas, { nombreZona: 'NombreZona' }, zonaInfoDiv)
        );

        // Mostrar valores ya seleccionados desde el inicio
        updateInfo(rutaSelect, rutas, { nombreRuta: 'NombreRuta' }, rutaInfoDiv);
        updateInfo(predioSelect, predios, {
            direccionPredio: 'direccion',
            distritoPredio: 'distrito',
