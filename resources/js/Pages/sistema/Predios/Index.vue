  ...
  <div class="container mx-auto p-4">
    ...
    <div class="d-flex justify-between align-items-center mb-4 flex-wrap gap-2">
      <form @submit.prevent="buscar" class="d-flex gap-2 flex-wrap">
        <input v-model="filters.search" type="text" placeholder="Buscar por dirección, barrio o distrito"
          class="form-control" />
      ...
      <div class="flex gap-2 flex-wrap">
        ...
        <Link :href="filters.deleted ? '/sistema/predios' : '/sistema/predios?deleted=true'"
    ...
    <table class="table-auto w-full border-collapse border border-gray-300">
      ...
      <tbody>
        <tr v-for="predio in predios.data" :key="predio.id" class="hover:bg-gray-100">
          <td class="border border-gray-300 px-4 py-2">{{ predio.id }}</td>
    ...
    <nav class="mt-4 flex justify-center">
      <ul class="pagination">
        <li v-for="(link, index) in predios.links" :key="index"
...
function eliminar(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(`/sistema/predios/${id}`, {
        preserveState: true,
      })
    }
  })
    ...
    if (result.isConfirmed) {
      router.post(`/sistema/predios/${id}/restore`, {}, {
        ...
        onSuccess: () => {
          ...
          // Recarga la lista con el filtro deleted=true
          router.get('/sistema/predios', { deleted: true, search: filters.value.search || '' }, {
            preserveState: true,
            replace: true,
          })
        },
