<template>
    <div class="container mx-auto p-4 max-w-5xl">
        ...
        <Link v-if="permissions.includes('usuarios.crear')" href="/sistema/usuarios/create"
        ...
        <tabla-busqueda titulo="Lista de Usuarios" fetch-url="/api/usuarios" :columnas="columnas" :per-page="10"
            ...
            <template #row="{ item }">
                ...
                <td class="p-2 border space-x-2">
                    <Link v-if="permissions.includes('usuarios.ver')" :href="`/sistema/usuarios/${item.id}`"
                        class="btn btn-info btn-sm px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700"
                        @click.stop>
                    Ver
                    </Link>
                    <Link v-if="permissions.includes('usuarios.editar')" :href="`/sistema/usuarios/${item.id}/edit`"
